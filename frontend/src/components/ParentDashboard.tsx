...
export default function ParentDashboard({ onLogout }: ParentDashboardProps) {
  ...
  const [chatHistory, setChatHistory] = useState<ChatMsg[]>([
    {
      role: "assistant",
      content: `Xin chào! Tôi là trợ lý ảo tích hợp dữ liệu từ CSDL nhà trường. Tôi có thể giúp Anh/Chị tra cứu **điểm số**, **nhận xét** từ giáo viên hoặc giải đáp thắc mắc về **quy chế học tập** của nhà trường.`,
    },
  ]);
  ...
  const fetchGrades = async (studentId: number) => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/v1/grades/student/${studentId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      setChatHistory((prev) => {
        ...
        updated[updated.length - 1] = {
          role: "assistant",
          content: "Xin lỗi, có lỗi khi kết nối với máy chủ.",
        };
        return updated;
      });
    } finally {
  ...
  const gradesByExamType = grades.reduce(
    (acc, g) => {
    ...
    <div className="bg-background text-on-surface min-h-screen flex flex-col overflow-hidden">
      ...
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 py-3 bg-white/70 backdrop-blur-md shadow-sm border-b border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <p className="text-xs font-medium text-on-surface-variant hidden md:block uppercase tracking-widest">
            Portal Phụ huynh
          </p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={onLogout}
            className="flex items-center gap-1 text-sm text-slate-500 hover:text-error font-semibold transition-colors"
          >
          ...
          <section className="col-span-12 lg:col-span-5 flex flex-col space-y-6 overflow-y-auto pr-2 custom-scrollbar">
            <div>
              <h1 className="text-2xl font-black tracking-tight text-on-surface">
                Báo cáo học tập & Phân tích AI
              </h1>
              <p className="text-on-surface-variant text-sm italic">
                Dữ liệu thời gian thực từ CSDL nhà trường
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {avgScore && parseFloat(avgScore) < 7 ? (
                <motion.div
                  ...
                  className="bg-error-container/5 border border-error/20 p-5 rounded-lg"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-[10px] bg-error text-white px-2 py-0.5 rounded-full">
                      Ưu tiên cao
                    </span>
                  </div>
                  <div className="mt-4">
                    <p className="text-sm font-bold text-on-surface">
                      Điểm trung bình đang thấp
                    </p>
                    <p className="text-xs text-on-surface-variant mt-1 leading-relaxed">
                      Điểm TB hiện tại: <strong>{avgScore}</strong>. Cần chú ý
                      hơn các môn học để cải thiện kết quả.
                    </p>
                  </div>
                </motion.div>
              ) : (
                <motion.div
                  whileHover={{ scale: 1.01 }}
                  className="bg-secondary-container/5 border border-secondary/20 p-5 rounded-lg"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                  ...
                  <div className="mt-4">
                    <p className="text-sm font-bold text-on-surface">
                      {avgScore
                        ? `Điểm TB: ${avgScore} — Học tập ổn định`
                        : "Đang tải dữ liệu điểm số..."}
                    </p>
                    <p className="text-xs text-on-surface-variant mt-1 leading-relaxed">
                      {grades.length > 0
                        ? `Đã cập nhật ${grades.length} bài kiểm tra từ CSDL.`
                        : "Liên hệ nhà trường để cập nhật điểm số."}
                    </p>
                  </div>
                </motion.div>
              )}
                    ...
                    {grades.length > 0 ? (
                      grades.map((g) => (
                        ...
                        >
                          ...
                          <td
                            className={`px-4 py-3 text-right font-bold ${g.score >= 8 ? "text-secondary" : g.score >= 5 ? "text-primary" : "text-error"}`}
                          >
                            {g.score}
                          </td>
                        </tr>
                      ...
                      <tr>
                        ...
                        >
                          {studentIds.length === 0
                            ? "Chưa có dữ liệu học sinh được liên kết."
                            : "Chưa có bài kiểm tra nào được nhập."}
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div className="p-3 bg-slate-50 border-t border-slate-100">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[9px] text-on-surface-variant italic leading-relaxed">
                    Dữ liệu được truy xuất trực tiếp từ CSDL nhà trường qua API
                    bảo mật.
                  </p>
                </div>
              </div>
