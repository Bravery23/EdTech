...
export default function StudentDashboard({ onLogout }: StudentDashboardProps) {
  ...
  // Data from backend
  const [mySubjectsData, setMySubjectsData] = useState<MySubjectsData | null>(
    null,
  );
  ...
  const startNewSession = async (ctx: SessionContext) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setExpandedGroups(
          (prev) => new Set([...prev, ctx.subject_name || "homeroom"]),
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === botMsgId
            ? { ...m, content: "Xin lỗi, có lỗi kết nối với máy chủ." }
            : m,
        ),
      );
    } finally {
  ...
  const sessionsBySubject = sessions.reduce(
    (acc, s) => {
  ...
  return (
    <div className="flex h-screen bg-surface overflow-hidden font-sans">
      {/* ===== SIDEBAR ===== */}
      <aside className="h-screen w-72 hidden md:flex flex-col border-r border-slate-100 bg-white z-40">
        {/* User Profile */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-[11px] text-on-surface-variant mt-0.5 truncate">
                {mySubjectsData?.class_name
                  ? `${mySubjectsData.class_name} • ${mySubjectsData.academic_year}`
                  : user?.email}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-1 custom-scrollbar">
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <p className="text-xs font-medium text-on-surface truncate">
                    {mySubjectsData.homeroom_teacher.teacher_name}
                  </p>
                </div>
              ...
              <div key={subj.subject_id} className="mb-1">
                ...
                >
                  ...
                  <div className="flex-1 text-left overflow-hidden">
                    <p className="text-[11px] font-black uppercase tracking-wider text-on-surface-variant truncate">
                      {subj.subject_name}
                    </p>
                  ...
                  <div className="ml-2 pl-4 border-l-2 border-slate-100 space-y-0.5 mt-1">
                    {subjectSessions.map((s) => (
                      <button
                        key={s.id}
                        onClick={() =>
                          loadSession(s.id, buildSubjectContext(subj, idx))
                        }
                        className={`w-full text-left px-3 py-2 rounded-lg text-xs transition-colors truncate ${
                          activeSessionId === s.id
                            ? "bg-slate-100 font-bold text-on-surface"
          ...
          {mySubjectsData &&
            mySubjectsData.subjects.length === 0 &&
            !mySubjectsData.homeroom_teacher && (
              <div className="px-4 py-8 text-center">
                <p className="text-xs text-slate-400 italic">
                  Bạn chưa được xếp lớp. Liên hệ admin.
                </p>
              </div>
            )}
        </div>
      ...
      <main className="flex-1 flex flex-col h-screen relative overflow-hidden">
        ...
        <header className="flex justify-between items-center px-6 py-3 bg-white/80 backdrop-blur-md border-b border-slate-100 z-30 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-slate-400 hover:text-primary hover:bg-blue-50 rounded-full transition-all"
            >
        ...
        {!activeSessionId ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            ...
            >
              ...
              <p className="text-on-surface-variant text-sm leading-relaxed mb-6">
                Chọn một môn học hoặc nhắn tin với Giáo viên chủ nhiệm để bắt
                đầu. Tôi sẽ hỗ trợ bạn theo phương pháp Socratic – gợi ý thay vì
                giải sẵn!
              </p>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {mySubjectsData.subjects.slice(0, 4).map((s, idx) => (
                    <button
                      key={s.subject_id}
                      onClick={() =>
                        startNewSession(buildSubjectContext(s, idx))
                      }
          ...
          <>
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar space-y-5 bg-slate-50/50"
            >
              <AnimatePresence>
                {messages.map((msg) => (
                  <motion.div
                  ...
                  >
                    ...
                    {/* Bubble */}
                    <div
                      className={`px-4 py-3 rounded-2xl shadow-sm text-sm leading-relaxed whitespace-pre-wrap max-w-xl ${
                        msg.role === "assistant"
            ...
            <div className="p-4 bg-white border-t border-slate-100 shrink-0">
              <div className="max-w-4xl mx-auto">
                <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden p-2 flex flex-col gap-2 focus-within:border-primary/30 transition-all">
                  {/* ===== FILE PREVIEW AREA ===== */}
                  <AnimatePresence>
                    {selectedFiles.length > 0 && (
                      ...
                      >
                        {selectedFiles.map((file, idx) => (
                          <motion.div
                            key={idx}
                            className="relative group flex items-center gap-2 bg-white border border-slate-200 rounded-lg p-1.5 pr-8 shadow-sm"
                          >
                            <div className="w-8 h-8 rounded-md bg-slate-100 flex items-center justify-center text-primary shrink-0">
                              {file.type.includes("image") ? (
                                ...
                                <BookOpen className="w-4 h-4" /> // Biểu tượng cho PDF
                              )}
                            </div>
                            <div className="flex flex-col overflow-hidden">
                              <span className="text-[10px] font-bold text-on-surface truncate max-w-[100px]">
                                {file.name}
                              </span>
                            ...
                            <button
                              ...
                              className="absolute right-1 top-1/2 -translate-y-1/2 p-1 text-slate-300 hover:text-error transition-colors"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </motion.div>
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <button
                      ...
                      className="p-2.5 text-slate-400 hover:text-primary hover:bg-blue-50 rounded-xl transition-all shrink-0"
                    >
                    ...
                    <textarea
                      ...
                      className="flex-1 focus:outline-none focus:ring-0 border-none bg-transparent text-sm py-2 max-h-32 custom-scrollbar placeholder:text-slate-400"
                ...
                <div className="flex items-center justify-between mt-2 px-1">
                  <p className="text-[10px] text-slate-300 font-medium italic">
                    Hỗ trợ ảnh bài tập & tài liệu PDF
                  </p>
                  {activeContext && (
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full text-white bg-gradient-to-r ${activeContext.color}`}
                    >
                      {activeContext.subject_name || "Chủ nhiệm"}
                    </span>
                  )}
                </div>
              </div>
            ...
            >
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-8 py-6 relative">
                ...
                <p className="text-blue-200 text-sm">
                  Chọn môn học hoặc giáo viên chủ nhiệm
                </p>
              </div>
              ...
              <div className="px-8 py-6 space-y-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-left flex-1">
                          <p className="font-black text-on-surface group-hover:text-white text-base transition-colors">
                            Giáo viên Chủ nhiệm
                          </p>
                          <p className="text-sm text-slate-400 group-hover:text-white/70 transition-colors">
                            {mySubjectsData.homeroom_teacher.teacher_name}
                          </p>
                        </div>
                ...
                <div className="space-y-2">
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-300 px-1">
                    Môn học
                  </p>
                  {mySubjectsData?.subjects.map((s, idx) => {
                    ...
                    return (
                      <button
                        ...
                        onMouseEnter={(e) => {
                          (
                            ...
                            `linear-gradient(135deg, var(--subject-from, #3b82f6), var(--subject-to, #4f46e5))`;
                        }}
                        onMouseLeave={(e) => {
                          (
                            e.currentTarget as HTMLButtonElement
                      ...
                      >
                        <div
                          className={`w-12 h-12 rounded-xl bg-gradient-to-br ${ctx.color} flex items-center justify-center shadow-md shrink-0`}
                        >
                          <BookOpen className="w-6 h-6 text-white" />
                        </div>
                        <div className="text-left flex-1 overflow-hidden">
                          <p className="font-black text-on-surface group-hover:text-white text-base transition-colors truncate">
                            {s.subject_name}
                          </p>
                          <p className="text-sm text-slate-400 group-hover:text-white/70 transition-colors truncate">
                            {s.teacher_name}
                          </p>
                        </div>
                  ...
                  !mySubjectsData.homeroom_teacher && (
                    <div className="text-center py-8">
                      <p className="text-slate-400 text-sm">
                        Bạn chưa được xếp lớp hoặc phân công môn học.
                      </p>
